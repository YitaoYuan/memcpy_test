...
void repeate_memcpy_test(size_t n, void *rbuf, void *sbuf, size_t stride, size_t size, size_t size_alloc)
{
    size_t segs = size_alloc / stride;
    for(size_t i = 0; i < n; i++) {
        size_t seg = i % segs;
        memcpy_test((char*)rbuf + stride * seg, (char*)sbuf + stride * seg, size);
    }
}
...
int main(int argc, char **argv)
{
    ...
    int node = 0;
    void *src = malloc_touched(size_alloc, node);
    void *dst = malloc_touched(size_alloc, node);
    ...
    for(int i = 0; i < nthread; i++) {
        ...
        mem_pool.push_back((memcpy_info){(char*)dst + i * size, (char*)src + i * size, nthread * size, cpuid, node});
    }
    uint64_t start = gettime_ns();
    std::vector<std::thread> thread_pool;
    for(int i = 0; i < nthread; i++) {
        thread t = std::thread(repeate_memcpy_test, niter, mem_pool[i].rbuf, mem_pool[i].sbuf, mem_pool[i].stride, size, size_alloc);
